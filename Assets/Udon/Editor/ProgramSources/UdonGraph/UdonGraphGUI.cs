    ...
    public class UdonGraphGUI : GraphGUI
    {
        ...
        private void DrawDocumentationLink(UdonNodeDefinition nodeDefinition)
        {
            ...
            if (nodeDefinition.fullName.Contains("Array.__ctor"))
            {
                ...
                string url = "https://docs.microsoft.com/en-us/dotnet/api/system.array?view=netframework-4.8";
                Help.BrowseURL(url);
                return;
            }
            ...
            {
                string url =
                    "https://docs.microsoft.com/en-us/dotnet/csharp/language-reference/operators/member-access-operators#indexer-operator-";
                Help.BrowseURL(url);
                return;
            }
            ...
            if (nodeDefinition.type.Namespace.Contains("System"))
            {
                ...
                if (nodeDefinition.name == "ctor")
                {
                    ...
                    foreach (Type pType in nodeDefinition.inputs)
                    {
                        url += "System_" + pType.Name.Replace('[', '_').Replace(']', '_') + "_";
                    }
        ...
        public override void NodeGUI(Node node)
        {
            ...
            for (int i = 0; i < node.inputSlots.Count(); i++)
            {
                ...
                {
                    if (!nodeName.StartsWithCached("Const_") && !nodeName.StartsWithCached("Variable_") && (nodeName != "Event_Custom" && nodeName != "Set_Variable" &&
                                                           nodeName != "Get_Variable"|| i > 1))
                    {
                        if (nodeName != "SubGraph" || !slot.isDataSlot)
                        {
                            LayoutSlot(
                                slot,
                                "",
        ...
        //                ((UdonGraph) graph).ReSerializeData();
                ...
                {
                    GUI.backgroundColor = new Color(.85f, .85f, .85f);
                    LayoutSlot(slot, slot.dataTypeString == "" ? slot.title : "", false, true, false, slot.name == "" || slot.dataTypeString == "" ? Styles.triggerPinIn : Styles.varPinIn);
                    Rect offsetRect = EditorGUILayout.GetControlRect(GUILayout.ExpandWidth(false),
                        GUILayout.MinWidth(160), GUILayout.MaxWidth(160));
        ...
        private float OutputsWidth(UdonNode node)
        {
            ...
            foreach (Slot slot in node.outputSlots)
            {
                ...
                if (udonNodeDefinition.outputNames != null && udonNodeDefinition.outputNames.Length > index &&
                    index >= 0)
                {
                    label = $"{udonNodeDefinition.outputNames[index]}"; //{label}
                }
                ...
                if (slot.isFlowSlot)
                {
                    ...
                    if (udonNodeDefinition.outputFlowNames != null &&
                        udonNodeDefinition.outputFlowNames.Length > flowIndex && flowIndex >= 0)
                    {
        ...
        private void DrawOutputSlot(Slot slot)
        {
            ...
            string label = ""; // slot.name;
            if (udonNodeDefinition.outputNames != null && udonNodeDefinition.outputNames.Length > index && index >= 0)
            {
                if (udonNodeDefinition.outputNames[index] == "T" || udonNodeDefinition.outputNames[index] == "T[]")
                {
                    label = udonNodeDefinition.outputNames[index];
                }
                else
                {
                    label = $"{udonNodeDefinition.outputNames[index]}"; //{label} 
        ...
        private void DrawSlotProperty(Slot slot, Rect rect, bool showField = true)
        {
            ...
            string label = "";
            if (udonNodeDefinition.inputNames != null && udonNodeDefinition.inputNames.Length > index && index >= 0)
            {
                label = udonNodeDefinition.inputNames[index];
            }
            ...
            else
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    //if (slot.node.name.StartsWithCached("Const_") && slot.node.inputSlots.ElementAt(1) == slot) //const name field 
                    //{
        ...
        private static object DrawSlotLabel(object value, Type type, Rect rect, string title, string uid)
        {
            ...
            {
                GUI.SetNextControlName("NodeField");
                string result = EditorGUI.TextField(rect, title, value.ToString());
                    ...
                    if (valueArray != null && valueArray.Length > 0)
                    {
                        for (int i = 0; i < valueArray.Length; i++)
                        {
                            ...
                            string result = EditorGUILayout.TextField($"{i}:",
                                valueArray.Length > i ? valueArray[i].ToString() : " ");
