...
namespace VRC.Udon
{
    public class UdonBehaviour : VRC.SDKBase.VRC_Interactable, ISerializationCallbackReceiver, IUdonEventReceiver, IUdonSyncTarget
    {
        ...
        private Dictionary<string, (object value, Type type)> publicVariables = new Dictionary<string, (object value, Type type)>();
        ...
        public void RunEditorUpdate(ref bool dirty)
        {
            ...
            programSource.RunProgramSourceEditor(publicVariables, ref dirty);
        ...
        private void DrawSerializationDebug()
        {
            ...
            bool newShowSerializedProgramJSON = EditorGUILayout.Foldout(showSerializedProgramJSON, "UdonBehaviour Serialized Program JSON");
            ...
            if(string.IsNullOrEmpty(serializedProgramBytesString))
            {
                return;
            }
            ...
            {
                EditorGUILayout.TextArea(System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(serializedProgramBytesString)));
            }
        ...
        public void RefreshProgram()
        {
            ...
            program = programSource.GetProgram();
            ...
            foreach(KeyValuePair<string, (object value, Type type)> publicVariable in publicVariables)
            {
                if(!symbolTable.HasAddressForSymbol(publicVariable.Key))
                {
                    continue;
                }

                uint symbolAddress = symbolTable.GetAddressFromSymbol(publicVariable.Key);
                (object value, Type declaredType) = publicVariable.Value;
            ...
            UnityEngine.Object unityEngineObjectValue = (UnityEngine.Object)this;
            if(unityEngineObjectValue == null || gameObject == null) //Catch a destroyed object null ref
            {
                return;
            }
        ...
        public override void Start()
        {
            ...
            {
                enabled = false;
                VRC.Core.Logger.Log($"No program; the UdonBehaviour on '{gameObject.name}' will not run.", _debugLevel, this);
                return;
            }
        ...
        {
            if(RunProgramAsRPCHook != null)
                RunProgramAsRPCHook(this, target, eventName);
        }
        ...
        private void RunProgram(uint entryPoint)
        {
            ...
            try
            {
                ...
                if(result != 0)
                {
                    VRC.Core.Logger.LogError($"Udon VM returned with error code: {result}", _debugLevel, this);
                }
            }
            catch(UdonVMException e)
            {
                VRC.Core.Logger.LogError($"{e}", _debugLevel, this);
            }

            if(originalAddress < 0xFFFFFFFF)
            {
        ...
        private void DeserializePublicVariables()
        {
            ...
            publicVariables = SerializationUtility.DeserializeValue<Dictionary<string, (object value, Type type)>>(
        ...
        private void DeserializeProgram()
        {
            byte[] serializedProgramBytes = Convert.FromBase64String(serializedProgramBytesString ?? "");
            program = SerializationUtility.DeserializeValue<IUdonProgram>(serializedProgramBytes, DataFormat.JSON, programUnityEngineObjects);
        }
        ...
        private void SerializeProgram()
        {
            byte[] serializedProgramBytes = SerializationUtility.SerializeValue(program, DataFormat.JSON, out programUnityEngineObjects);
            serializedProgramBytesString = Convert.ToBase64String(serializedProgramBytes);
        }

        private void SerializePublicVariables()
        {
            byte[] serializedPublicVariablesBytes = SerializationUtility.SerializeValue(publicVariables, DataFormat.Binary, out publicVariablesUnityEngineObjects);
