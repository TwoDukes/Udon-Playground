...
public partial class VRCSdkControlPanel : EditorWindow
{
    ...
    void OnGUIAvatarCheck(VRC.SDKBase.VRC_AvatarDescriptor avatar)
    {
        ...
        List<VRC.SDKBase.VRC_Station> stations = avatar.gameObject.GetComponentsInChildren<VRC.SDKBase.VRC_Station>(true).ToList<VRC.SDKBase.VRC_Station>();
        if (stations.Count > 0)
            OnGUIWarning(avatar, "Stations found on Avatar, they will be adjusted to safe limits, if necessary.",
                GetAvatarSubSelectAction(avatar, typeof(VRC.SDKBase.VRC_Station)), null);
